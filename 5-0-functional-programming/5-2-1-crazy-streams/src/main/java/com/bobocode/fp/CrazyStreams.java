...
public class CrazyStreams {
    ...
    public List<Account> findAccountsByBirthdayMonth(Month birthdayMonth) {
        return accounts.stream().filter(a -> a.getBirthday().getMonth().equals(birthdayMonth)).collect(Collectors.toList());
    }
    ...
    public Map<Boolean, List<Account>> partitionMaleAccounts() {
        return accounts.stream().collect(Collectors.partitioningBy(account -> account.getSex() == Sex.MALE, Collectors.toList()));
    }
    ...
    public Map<String, List<Account>> groupAccountsByEmailDomain() {
        return accounts.stream().collect(Collectors.groupingBy(account -> account.getEmail().split("@")[1], Collectors.toList()));
    }
    ...
    public int getNumOfLettersInFirstAndLastNames() {
        return accounts.stream().mapToInt(a -> a.getFirstName().length() + a.getLastName().length()).reduce(0, Integer::sum);
    }
    ...
    public boolean containsAccountWithEmailDomain(String emailDomain) {
        return accounts.stream().anyMatch(a -> a.getEmail().contains(emailDomain));
    }
